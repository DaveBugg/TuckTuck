...
# ─────────────── Уборка за собой ───────────────
#
# Каждое обновление переставляет тег latest на новый образ, а прежний остаётся
# лежать без тега. Сами по себе такие образы не удаляются никогда: за несколько
# десятков обновлений накапливаются десятки гигабайт, и диск заканчивается
# ровно на очередном деплое — то есть в самый неподходящий момент.
#
# Чистим ТОЛЬКО безтеговые (dangling) и ТОЛЬКО после успешной проверки: до неё
# ещё возможен откат, а тегированные образы прежних сборок нужны, чтобы откат
# не требовал похода в реестр.
if [ "${TUCKTUCK_KEEP_IMAGES:-0}" != "1" ]; then
  FREED="$(docker image prune -f 2>/dev/null | sed -n 's/^Total reclaimed space: //p')"
  case "${FREED:-0B}" in
    0B|"") : ;;
    *) log "Убрал образы прежних сборок, освобождено $FREED" ;;
  esac
fi
