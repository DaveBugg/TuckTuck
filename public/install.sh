...
#
# Обновление уже установленного агента — без переменных вовсе:
#   curl -fsSL https://ПАНЕЛЬ/install.sh | sh
#
...
# Адрес и токен можно не передавать, если агент здесь уже стоит: берём их из
# его же настроек. Это и есть обновление — скачать свежий скрипт и переставить
# расписание, не выпуская новый токен и не идя за старым в панель.
if [ -z "${TUCKTUCK_URL:-}" ] || [ -z "${TUCKTUCK_TOKEN:-}" ]; then
  if [ -r "$ENVF" ]; then
    # Читаем ровно две строки, а не подключаем файл целиком: в него мог попасть
    # посторонний мусор, и выполнять его содержимое как код незачем.
    [ -n "${TUCKTUCK_URL:-}" ]   || TUCKTUCK_URL="$(sed -n 's/^TUCKTUCK_URL=//p' "$ENVF" | head -1)"
    [ -n "${TUCKTUCK_TOKEN:-}" ] || TUCKTUCK_TOKEN="$(sed -n 's/^TUCKTUCK_TOKEN=//p' "$ENVF" | head -1)"
    # Именно if, а не «условие && команда»: при set -e невыполнившееся условие
    # в конце такой цепочки завершает весь скрипт с ошибкой.
    if [ -n "${TUCKTUCK_URL:-}" ] && [ -n "${TUCKTUCK_TOKEN:-}" ]; then
      step "Обновляю уже установленный агент: настройки взяты из $ENVF"
    fi
  fi
fi
