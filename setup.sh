...
#   TUCKTUCK_KEEP_IMAGES  1 — не убирать образы прежних сборок после обновления
#
...
# Уборка за собой: обновление переставляет тег latest на новый образ, а прежний
# остаётся лежать без тега и не удаляется никогда. За несколько десятков
# обновлений это десятки гигабайт, и диск кончается ровно на очередном деплое.
# Чистим только безтеговые и только после успешной проверки.
if [ "${TUCKTUCK_KEEP_IMAGES:-0}" != "1" ]; then
  FREED="$(docker image prune -f </dev/null 2>/dev/null | sed -n 's/^Total reclaimed space: //p')"
  case "${FREED:-0B}" in
    0B|"") : ;;
    *) ok "убраны образы прежних сборок, освобождено $FREED" ;;
  esac
fi
